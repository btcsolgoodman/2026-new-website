...
function tawkHideMatch(node) {
  ...
  // div — 내용 텍스트로만 판단 (main widget 안 건드림)
  if (tag === 'DIV') {
    var text = (node.textContent || '').toLowerCase();
    if (text.indexOf('we are here') > -1 ||
        text.indexOf("we're here") > -1) return true;
    var html = (node.innerHTML || '').toLowerCase();
    if (html.indexOf('attention-grabber') > -1 ||
        html.indexOf('proactive-chat') > -1) return true;
  }
